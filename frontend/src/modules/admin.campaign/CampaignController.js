...
export default class CampaignController {
  ...
  $onInit() {
    ...
    this.$scope.availableFrontendTranslations =
      this.DataService.getAvailableFrontendTranslations();
    ...
    this.connectType = [
      ...
      },
      {
        name: this.$filter("translate")(
          "campaign.connect_type.qrcode_earning_rule"
        ),
        type: "qrcode",
      },
      {
        name: this.$filter("translate")(
          "campaign.connect_type.geolocation_earning_rule"
        ),
        type: "geolocation",
      },
    ];
    ...
    let segmentPromise = this.SegmentService.getActiveSegments({
      perPage: 1000,
    }).then((res) => {
      this.segments = res;
    });

    let levelPromise = this.LevelService.getLevels({ perPage: 1000 }).then(
      (res) => {
        this.levels = res;
      }
    );

    let posPromise = this.PosService.getPos({ perPage: 1000 }).then((res) => {
      this.pos = res;
    });

    this.dataPromise = this.$q.all([
      segmentPromise,
      levelPromise,
      categoryPromise,
      posPromise,
    ...
    self.tableParams = new self.NgTableParams(
      ...
      },
      {
        getData: function (params) {
          ...
          self.CampaignService.getCampaigns(
  ...
  getConnectTypeEarningRule(connectType) {
    ...
    function deleteEarningRuleIdFieldData() {
      ...
      if (
        self.$scope.editableFields &&
        self.$scope.editableFields.earningRuleId
      ) {
    ...
    if (connectType == "none") {
      ...
      self.CampaignService.getConnectTypeEarningRule(connectType).then(
        (res) => {
  ...
  setCategoryState(campaignCategoryId, active) {
    ...
    self.CampaignService.setCategoryState(campaignCategoryId, active).then(
      () => {
        let message = self.$filter("translate")(
          "xhr.post_activate_campaign_category.success"
        );
      ...
      () => {
        let message = self.$filter("translate")(
          "xhr.post_activate_campaign_category.error"
        );
        self.Flash.create("danger", message);
      }
    );
    ...
    self.categoryTableParams = new self.NgTableParams(
      ...
      },
      {
        getData: function (params) {
          ...
          self.CampaignService.getCategories(
            self.ParamsMap.params(params.url())
          ).then(
            (res) => {
            ...
            () => {
              let message = self.$filter("translate")(
                "xhr.get_campaign_categories.error"
              );
              self.loaderStates.categoryList = false;
              self.loaderStates.coverLoader = false;
              self.Flash.create("danger", message);
  ...
  getCategoryDetails() {
    ...
    if (self.campaignCategoryId) {
      ...
      let message = self.$filter("translate")(
        "xhr.get_campaign_category.no_id"
      );
  ...
  addCategory(newCategory) {
    ...
    self.CampaignService.postCategory(newCategory).then(
      (res) => {
        let message = self.$filter("translate")(
          "xhr.post_campaign_category.success"
        );
        self.Flash.create("success", message);
        self.loaderStates.coverLoader = false;
        self.$state.go("admin.campaign-category-list");
      ...
      (res) => {
        ...
        let message = self.$filter("translate")(
          "xhr.post_campaign_category.error"
        );
        self.Flash.create("danger", message);
  ...
  editCategory(editedCategory) {
    ...
    self.CampaignService.putCategory(
      self.campaignCategoryId,
      editedCategory
    ).then(
      (res) => {
        let message = self.$filter("translate")(
          "xhr.put_campaign_category.success"
        );
        self.Flash.create("success", message);
        self.loaderStates.coverLoader = false;
        self.$state.go("admin.campaign-category-list");
      ...
      (res) => {
        self.$scope.validate = self.Validation.mapSymfonyValidation(res.data);
        let message = self.$filter("translate")(
          "xhr.put_campaign_category.error"
        );
        self.Flash.create("danger", message);
        self.loaderStates.categoryDetails = false;
        self.loaderStates.coverLoader = false;
  ...
  setCampaignState(active, campaignId) {
    ...
    self.CampaignService.setCampaignState(active, campaignId).then(
      () => {
        let message = self.$filter("translate")(
          "xhr.post_activate_campaign.success"
        );
        self.Flash.create("success", message);
      ...
      () => {
        let message = self.$filter("translate")(
          "xhr.post_activate_campaign.error"
        );
        self.Flash.create("danger", message);
      }
    );
  ...
  addCampaign(newCampaign) {
    ...
    self.CampaignService.postCampaign(newCampaign).then(
      (res) => {
  ...
  deletePhoto(photoId) {
    ...
    this.CampaignService.deleteCampaignImage(
      this.$stateParams.campaignId,
      photoId
    )
      .then((res) => {
        let message = self.$filter("translate")(
          "xhr.delete_campaign_image.success"
        );
        self.Flash.create("success", message);
      ...
      .catch((err) => {
        self.$scope.validate = self.Validation.mapSymfonyValidation(res.data);
        let message = self.$filter("translate")(
          "xhr.delete_campaign_image.error"
        );
        self.Flash.create("danger", message);
      });
  }
  ...
  deleteBrandIcon() {
    ...
    this.CampaignService.deleteCampaignBrandIcon(this.$stateParams.campaignId)
      .then((res) => {
        ...
        let message = self.$filter("translate")(
          "xhr.delete_campaign_brand_icon.success"
        );
        self.Flash.create("success", message);
      })
      .catch((err) => {
        self.$scope.validate = self.Validation.mapSymfonyValidation(res.data);
        let message = self.$filter("translate")(
          "xhr.delete_campaign_brand_icon.error"
        );
        self.Flash.create("danger", message);
      });
  }
  ...
  editCampaign(editedCampaign) {
    ...
    for (let property in editedCampaign) {
      if (editedCampaign.hasOwnProperty(property)) {
        if (
          typeof self.$scope.editableFields[property] === "undefined" ||
    ...
    self.CampaignService.putCampaign(editedCampaign).then(
      (res) => {
  ...
  _uploadCampaignFile(res, method) {
    ...
    function postCampaignImages(item, key) {
      return self.CampaignService.postCampaignImages(campaignId, item.file)
        ...
        .catch((err) => {
          ...
          self.$scope.CampaignCtrl.campaignFiles[key].error =
            inputError.file.errors[0];
          let message = self.$filter("translate")("xhr.put_campaign.error");
          self.Flash.create("danger", message);
          self.loaderStates.coverLoader = false;
        });
    ...
    function postCampaignBrandIcon() {
      return self.CampaignService.postCampaignBrandIcon(
        campaignId,
        self.$scope.campaignBrandIconFile
      )
        .then((res2) => {})
        .catch((err) => {
          self.$scope.brandIconFileValidate =
            self.Validation.mapSymfonyValidation(err.data);

          let message = self.$filter("translate")(
            "xhr." + method + "_campaign_brand_icon.warning"
          );
    ...
    if (self.$scope.CampaignCtrl.campaignFiles) {
      ...
      angular.forEach(
        self.$scope.CampaignCtrl.campaignFiles,
        function (item, key) {
          if (item.file.name) {
    ...
    if (queries.length > 0) {
      self.$q.all(queries).then(function () {
        ...
        if (
          self.$scope.CampaignCtrl.campaignFiles &&
          self.$scope.fileValidate.length
        ) {
          failed = true;
        }
        if (
  ...
  getCustomersForCampaign() {
    ...
    if (self.campaignId) {
      self.customersTableParams = new self.NgTableParams(
        ...
        },
        {
          getData: function (params) {
            ...
            self.CampaignService.getVisibleCustomers(
              self.campaignId,
              self.ParamsMap.params(params.url())
            ).then(
              (res) => {
              ...
              () => {
                let message = self.$filter("translate")(
                  "xhr.get_customers_for_campaign.error"
                );
                self.Flash.create("danger", message);
                dfd.reject();
                self.loaderStates.campaignCustomerList = false;
      ...
      self.$state.go("admin.campaign-list");
      let message = self.$filter("translate")(
        "xhr.get_customers_for_campaign.no_id"
      );
      self.Flash.create("warning", message);
      self.loaderStates.campaignCustomerList = false;
    }
  ...
  generatePhotoRoute() {
    return (
      this.DataService.getConfig().apiUrl +
      "/campaign/" +
      this.$stateParams.campaignId +
      "/photo"
    );
  ...
  _getCampaign() {
    ...
    self.CampaignService.getCampaign(self.campaignId).then(
      (res) => {
        self.$scope.campaign = res;
        self.$scope.editableFields = self.EditableMap.humanizeCampaign(res);

        if (
          self.$scope.editableFields.levels &&
          self.$scope.editableFields.levels.length
        ) {
        ...
        if (
          self.$scope.editableFields.pos &&
          self.$scope.editableFields.pos.length
        ) {
          let pos = self.$scope.editableFields.pos;
          for (let i in pos) {
            let pos = _.find(self.pos, { id: pos[i] });
          }
        }
        if (
          self.$scope.editableFields.segments &&
          self.$scope.editableFields.segments.length
        ) {
        ...
        if (
          self.$scope.editableFields.categories &&
          self.$scope.editableFields.categories.length
        ) {
        ...
        if (self.$scope.editableFields.connectType) {
          self.getConnectTypeEarningRule(
            self.$scope.editableFields.connectType
          );
        }
      },
      () => {
  ...
  _getCategory() {
    ...
    self.CampaignService.getCategory(self.campaignCategoryId).then(
      (res) => {
        self.$scope.category = res;
        self.$scope.editableCategoryFields =
          self.EditableMap.humanizeCampaignCategory(res);
        self.loaderStates.categoryDetails = false;
      },
      () => {
        let message = self.$filter("translate")(
          "xhr.get_campaign_category.error"
        );
        self.Flash.create("danger", message);
        self.loaderStates.categoryDetails = false;
      }
  ...
  getDeliveryStatuses() {
    ...
    self.CampaignService.getDeliveryStatuses().then(
      (res) => {
        ...
        for (var value of res.choices) {
          self.deliveryStatusesHumanized.push({
            value: value,
            label: self.$filter("translate")(
              `campaign.delivery_statuses.${value}`
            ),
          });
      ...
      () => {
        let message = self.$filter("translate")(
          "xhr.get_delivery_statuses.error"
        );
        self.Flash.create("danger", message);
        self.loaderStates.deliveryStatuses = false;
      }
  ...
  setDeliveryStatus(status, customerId, couponId) {
    ...
    self.CampaignService.putDeliveryStatus(status, customerId, couponId).then(
      () => {
        let message = self.$filter("translate")(
          "xhr.put_change_delivery_status.success"
        );
        self.Flash.create("success", message);
      },
      () => {
        let message = self.$filter("translate")(
          "xhr.put_change_delivery_status.error"
        );
        self.Flash.create("danger", message);
      }
    );
      ...
      },
      {
        getData: function (params) {
          ...
          if (parsedParams.purchasedAtFrom) {
            parsedParams.purchasedAtFrom = new Date(
              decodeURIComponent(parsedParams.purchasedAtFrom)
            );
          }

          if (parsedParams.purchasedAtTo) {
            parsedParams.purchasedAtTo = new Date(
              decodeURIComponent(parsedParams.purchasedAtTo)
            );
          }

          if (parsedParams.usageDateFrom) {
            parsedParams.usageDateFrom = new Date(
              decodeURIComponent(parsedParams.usageDateFrom)
            );
          }

          if (parsedParams.usageDateTo) {
            parsedParams.usageDateTo = new Date(
              decodeURIComponent(parsedParams.usageDateTo)
            );
          }

          self.loaderStates.redeemedCampaigns = true;
          self.CampaignService.getRedeemedCampaignRewards(parsedParams).then(
            (res) => {
            ...
            () => {
              let message = self.$filter("translate")(
                "xhr.get_redeemed_campaigns.error"
              );
              self.loaderStates.redeemedCampaigns = false;
              self.loaderStates.coverLoader = false;
              self.Flash.create("danger", message);
  ...
  updateCampaignUsage(customerId, campaignId, code, couponId, used) {
    let self = this;
    self.CustomerService.postUsage(
      customerId,
      campaignId,
      code,
      couponId,
  ...
  canBeBoughtManually(campaign) {
    return (
      campaign.active &&
      campaign.reward !== "cashback" &&
      campaign.reward !== "custom_campaign_code"
    );
  }
  ...
  buyCampaignManuallyByAdmin() {
    ...
    if (self.$scope.buyCampaignManually.customerId) {
      self.CampaignService.postBuyCampaignManually(
        self.$scope.buyCampaignManually
      ).then(
        () => {
          let message = self.$filter("translate")(
            "xhr.post_campaign_buy_manually_assign.success"
          );
          self.Flash.create("success", message);
          this.$scope.buyCampaignManuallyModal = false;
        ...
        (res) => {
          ...
          if (res.status === 400) {
            message =
              self.$filter("translate")(
                "xhr.post_campaign_buy_manually_assign.error_occurred"
              ) +
              ": " +
              res.data.error;
          } else {
            message = self.$filter("translate")(
              "xhr.post_campaign_buy_manually_assign.error"
            );
          }
          self.Flash.create("danger", message);
          this.$scope.buyCampaignManuallyModal = false;
  ...
  _selectizeConfigs() {
    ...
    this.campaignsUsageSelectOptions = [
      ...
      },
      {
        ...
        label: self.$filter("translate")(
          "campaign.usage_types." + USAGE_TYPE_DELIVERED
        ),
      },
      {
        value: USAGE_TYPE_USED,
        label: self.$filter("translate")(
          "campaign.usage_types." + USAGE_TYPE_USED
        ),
      },
    ];
    ...
    this.customersConfig = {
      ...
      placeholder: this.$filter("translate")(
        "global.start_typing_an_email_or_phone"
      ),
      onChange: () => {
      ...
      load: (query, callback) => {
        ...
        self.CustomerService.getCustomers(
